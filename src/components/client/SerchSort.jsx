...
const SerchSort = () => {
    ...
    const [isClient, setIsClient] = useState(false);
    const [dropdownOpen, setDropdownOpen] = useState(false); // для выпадающего списка
        ...
        const fetchCategories = async () => {
            try {
                ...
                if (!response.ok)
                    throw new Error(`HTTP error! status: ${response.status}`);
                const data = await response.json();
                setCategories(["Все", ...data]); // "Все" всегда первым
            } catch (error) {
                console.error("Error:", error);
            }
        };
    ...
    // Поиск товаров при изменении текста или категорий
    useEffect(() => {
        const controller = new AbortController();
        const fetchData = async () => {
            try {
                const searchParams = new URLSearchParams();
                if (serchData.text) searchParams.append("text", serchData.text);
                if (serchData.categor && serchData.categor.length > 0)
                    searchParams.append(
                        "categor",
                        JSON.stringify(serchData.categor)
    ...
    const toggleCategory = (categoryName) => {
        setSerchData((prev) => {
            ...
            if (newState.categor.length === 0) newState.categor = ["Все"];
            return newState;
        });
    };

    const toggleText = (text) => setSerchData((prev) => ({ ...prev, text }));

    return (
        <>
            {/* Верхняя панель */}
            <div className="bg-white sticky top-0 z-50 dark:bg-zinc-100">
                <div className="flex p-2 md:p-4 items-center gap-1 md:gap-6 max-w-7xl mx-auto">
                    Лого
                    {/* <div href="#" className="flex gap-2 cursor-pointer"> */}
                    <svg className="w-8" viewBox="0 0 24 24" fill="none">
            ...
            <div className="max-w-7xl mx-auto mt-4 px-2 md:px-0 relative">
                <button
                    onClick={() => setDropdownOpen(!dropdownOpen)}
                    className="px-4 py-2 rounded-full bg-teal-600 text-white hover:bg-teal-800 transition shadow"
                >
                    Выбрать категории
                </button>

                {dropdownOpen && (
                    <div className="absolute mt-2 w-full md:w-96 bg-gray-200 rounded-xl shadow-lg p-3 flex flex-wrap gap-2 z-50">
                        {categories.map((item, index) => {
                            const isSelected = serchData.categor.includes(item);
                            return (
                                <div
